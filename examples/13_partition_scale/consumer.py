...
def worker_1(msg):
    print(f"👷 [WORKER 1] Processed job {msg.value.get('job_id')} on partition offset {msg.offset}")


# Worker 2
@kafka.consumer(topic=TOPIC_NAME, format="json")
def worker_2(msg):
    print(f"👷 [WORKER 2] Processed job {msg.value.get('job_id')} on partition offset {msg.offset}")
